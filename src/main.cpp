...
int main(int argc, char **argv) {
  ...
  programm.add_argument("input").help("Audiofile to stretch");

  programm.add_argument("--stretchfactor", "-s")
      .help("Time stretch factor; >1 means slower audio.")
      .default_value(1.5f)
      .scan<'f', float>();

  programm.add_argument("--output", "-o")
      .help("output folder")
      .default_value(currentFolder.string());

  programm.add_argument("--framesize", "-f")
      .help("The size of the analysis frame for the stretching algorithm")
      .default_value(512)
      .scan<'i', int>();
  ...
  PLOG_DEBUG << std::format(
      "Running Audiostretcher2 with file {0} outputting to {1} with stretching "
      "factor {2}. It has {3} channels ",
      inputfile.string(), outputfolder.string(), stretchingFactor,
      inputInfo.channels);

  // open the sound file
  audiofile = sf_open(inputfile.string().c_str(), SFM_READ, &inputInfo);
  if (sf_error(audiofile)) {
    PLOGE << std::format("Error opening the audio file with error:  {}",
                         sf_strerror(audiofile));
  }

  if (inputInfo.channels > 2) {
    PLOG_ERROR << "Audio file has more than 2 channels. Only mono or stereo "
  ...
  PLOGD << std::format("configured {0}  channels with {1} samples",
                       inputAudiochannels.size(),
                       inputAudiochannels.at(0).size());
  ...
  sf_close(audiofile);
  if (audiostretch::deinterleaveArray<float>(tempAudio, inputAudiochannels,
                                             inputInfo.channels)) {
